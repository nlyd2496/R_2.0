...
{
    public class R036GG1K1 : KMonoBehaviour, ISingleSliderControl, ISliderControl
    {
        public float GetSliderMin(int index) => 0;//最小值
        public float GetSliderMax(int index) => 100000;//最大值
        public int SliderDecimalPlaces(int index) => 0;//小数点
        public string SliderUnits => UI.UNITSUFFIXES.LIGHT.LUX;
        public string SliderTitleKey => "STRINGS.BUILDINGS.PREFABS.CEILINGLIGHT.NAME";//窗口名称
        public string GetSliderTooltip(int index) => "";//空
        //--------------------------
        [Serialize] public float AA = 2000f;//AA参数默认值
        //--------------------------
        ...
        internal void OnCopySettings(object data) { var component = ((GameObject)data).GetComponent<R036GG1K1>(); if (component == null) return; AA = component.AA; Update(); }
        //--------------------------
    }
}

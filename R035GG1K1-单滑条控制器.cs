...
{
    public class R035GG1K1 : KMonoBehaviour, ISingleSliderControl, ISliderControl
    {
        ...
        public string SliderUnits => UI.UNITSUFFIXES.TILES; //单位
        public string SliderTitleKey => "STRINGS.BUILDINGS.PREFABS.R035GG1.NAME";//窗口名称
        public string GetSliderTooltip(int index) => "";//空
        //--------------------------
        [Serialize] public float AA = 8f;//AA参数默认值
        //--------------------------
        [MyCmpReq] public EnergyConsumer energyConsumer;//能量
        [MyCmpReq] public Light2D light2D;//灯
        internal void Update() 
        {
            this.light2D.Range = (int)this.AA;
            this.energyConsumer.BaseWattageRating = this.energyConsumer.WattsNeededWhenActive * 0.1f * this.AA;
            this.light2D.Lux = (int)(this.AA * 200f);
        }
        //将AA的值赋予源功能组件
        //--------------------------
        //以下内容无需更改
        ...
        internal void OnCopySettings(object data) { var component = ((GameObject)data).GetComponent<R035GG1K1>(); if (component == null) return; AA = component.AA; Update(); }
        //--------------------------
    }
}
